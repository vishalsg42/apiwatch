...
| 2026-08-29 | PR opened | [novuhq/novu#12490](https://github.com/novuhq/novu/pull/12490) `fix(worker)`: explicit timeout on the telemetry HttpModule. Opened **as a draft**, so it will not attract review until marked ready. GitHub had refused `CreatePullRequest` three times across a 3 minute backoff while `rate_limit` reported the full quota remaining, which reads as an abuse heuristic on a second PR from a fork created minutes earlier |
| 2026-08-29 | not a result | #12489 shows `APPROVED`, but the reviewer is `greptile-apps[bot]`, an automated code reviewer. Under the threshold above this counts for nothing: only a merge satisfies criterion 1. Recorded so a later reading of the PR list does not mistake it for traction. The two comments on each novu PR are the Vercel and Netlify deploy bots, and the one failing check on each is preview-deploy authorization for an external contributor, not a test |
